...
def train_optimize(X, y, conf, smell="", hpo="", n_splits=10, n_repeats=10, show_result=False, show_process=True, export_models=True, model_dir=""):
    ...
    sub_dir : str
        A path of sub-directory to be exported within the ./models directory 
    ...
    # export models
    if export_models:
        dt_clf.export_model(model_dir=model_dir, pre_fix=hpo)
        rf_clf.export_model(model_dir=model_dir, pre_fix=hpo)
...
def train_baseline(X, y, smell="", n_splits=10, n_repeats=10, show_result=False, show_process=True, export_models=True, model_dir=""):
    ...
    # export models
    if export_models:
        dt_clf.export_model(model_dir=model_dir, pre_fix="baseline")
        rf_clf.export_model(model_dir=model_dir, pre_fix="baseline")
